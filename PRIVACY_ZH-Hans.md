...
- **设置：** 您的配置（重命名模式、文件操作、语言选择）保存在本地 JSON 文件中。
- **文件夹书签：** 您所选择的输入和输出文件夹会以安全范围书签（security-scoped bookmarks）的形式保存，以便 App 在重新启动后仍能保留访问权限。
- **用户偏好设置：** 语言选择、欢迎界面状态等简单设置保存在本地 UserDefaults 中。
